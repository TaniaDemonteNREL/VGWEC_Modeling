function [w,B] = getRadiationDampingFromBEMIO(varargin,plotFlag,rho)
...
for ii=1:numHydro
    ...
    for i = 1:numBod
        m = varargin(ii).dof(i);
        w = X.X1;
        Y.(tmp2)(1,i,:) = squeeze(varargin(ii).B(a+1,a+1,:)) * rho .* w';
        Y.(tmp2)(2,i,:) = squeeze(varargin(ii).B(a+3,a+3,:)) * rho .* w';
        Y.(tmp2)(3,i,:) = squeeze(varargin(ii).B(a+5,a+5,:)) * rho .* w';
...
% w = X.X1;
B.Surge = squeeze(Y.Y1(1,1,:)) ;
B.Heave = squeeze(Y.Y1(2,1,:)) ;
B.Pitch = squeeze(Y.Y1(3,1,:)) ;
