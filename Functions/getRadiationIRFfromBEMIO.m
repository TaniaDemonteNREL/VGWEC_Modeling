function [t,radIRF] = getRadiationIRFfromBEMIO(varargin,plotFlag,rho)
...
titleString = ['Radiation Impulse Response Functions: ',...
    '$$\bar{K}_{i,j}(t) = {\frac{2}{\pi}}\int_0^{\infty}{B_{i,j}(\omega)}\cos({\omega}t)d\omega$$'];
...
for ii = 1:numHydro
    ...
    for iBod = 1:numBod
        ...
        Y.(tmp2)(1,i,:) = squeeze(varargin(ii).ra_K(a+1,a+1,:)) * rho ;
        Y.(tmp2)(2,i,:) = squeeze(varargin(ii).ra_K(a+3,a+3,:)) * rho ;
        Y.(tmp2)(3,i,:) = squeeze(varargin(ii).ra_K(a+5,a+5,:)) * rho ;
        legendStrings{i,ii} = [varargin(ii).body{iBod}];
        i = i+1;
        if isfield(varargin(ii),'ss_A')==1
            Y.(tmp2)(1,i,:) = squeeze(varargin(ii).ss_K(a+1,a+1,:)) * rho ;
            Y.(tmp2)(2,i,:) = squeeze(varargin(ii).ss_K(a+3,a+3,:)) * rho ;
            Y.(tmp2)(3,i,:) = squeeze(varargin(ii).ss_K(a+5,a+5,:)) * rho ;

%             Y.(tmp2)(3,i,:) = squeeze(varargin(ii).ss_A(a+5,a+5,:)) * rho ;
...
t = X.X1; 
radIRF.Surge = squeeze(Y.Y1(1,1,:));  % Surge
radIRF.Heave = squeeze(Y.Y1(2,1,:));  % Heave
radIRF.Pitch = squeeze(Y.Y1(3,1,:));  % Pitch

radIRF.SurgeSS = squeeze(Y.Y1(1,2,:));  % Surge
radIRF.HeaveSS = squeeze(Y.Y1(2,2,:));  % Heave
radIRF.PitchSS = squeeze(Y.Y1(3,2,:));  % Pitch
